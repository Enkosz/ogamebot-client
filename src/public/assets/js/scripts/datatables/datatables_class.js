$(document).ready(function() {
  // Endpoint API
  const endpointAPI = "/api/classes/list";
        ...
        render: function(data, type, row) {
          ...
          <span class="dropdown">
            ...
            <span aria-labelledby="dropdown-options-table" class="dropdown-menu mt-1 dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(62px, 31px, 0px);">
                ...
                <a href="/classes/edit?id=${data}" class="dropdown-item"><i class="la la-pencil"></i> Modifica</a>
                <a href="/classes/delete?id=${data}" class="dropdown-item" onClick="return confirm('Sei sicuro di voler eliminare questa classe?');"><i class="la la-trash"></i> Cancella</a>
            </span>
          </span>
          `;
        }
